...
client = OpenAI(
    ...
    base_url=os.getenv("QWEN_API_BASE", "BASE_URL")
)
...
chunks = text_split.text_split(file_path="../doc_preprocessed/X6_Electrical-Instruction_Data-Sheet_Manual_CHI.md")
for index, chunk in enumerate(chunks):
    ...
    try:
        ...
        print(f"JSON 解析错误: {e}--{index}")
        # 尝试修复生成的 JSON 字符串
        # fixed_content = response.choices[0].message.content.replace("\n", "\\n")
    ...
    save_kg_data(data=data, index=index, output_dir="kg_output/qwen")
